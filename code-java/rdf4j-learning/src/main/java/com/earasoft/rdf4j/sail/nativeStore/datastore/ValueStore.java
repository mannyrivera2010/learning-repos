 ...
 *******************************************************************************/
package com.earasoft.rdf4j.sail.nativeStore.datastore;
...
import com.earasoft.rdf4j.sail.rocksDbStore.cache.ConcurrentCache;
...
import com.earasoft.rdf4j.sail.rocksDbStore.model.NativeBNode;
import com.earasoft.rdf4j.sail.rocksDbStore.model.NativeIRI;
import com.earasoft.rdf4j.sail.rocksDbStore.model.NativeLiteral;
import com.earasoft.rdf4j.sail.rocksDbStore.model.NativeResource;
...
public class ValueStore extends AbstractValueFactory {
	...
	public ValueStore(RocksDbSailStore nativeSailStore, File dataDir) throws IOException {
		this(nativeSailStore, dataDir, false);
	}

	public ValueStore(RocksDbSailStore nativeSailStore, File dataDir, boolean forceSync) throws IOException {
		this(nativeSailStore, dataDir, forceSync, VALUE_CACHE_SIZE, VALUE_ID_CACHE_SIZE, NAMESPACE_CACHE_SIZE, NAMESPACE_ID_CACHE_SIZE);
	}

	public ValueStore(RocksDbSailStore nativeSailStore, File dataDir, boolean forceSync, int valueCacheSize, int valueIDCacheSize, int namespaceCacheSize,
...
//			System.out.print(data1.length);
	...
	public int getID(Value value) throws IOException {
		...
		// Check to see if value object is a native value
		//
		...
		byte[] data = value2data(this, value, false);
	...
	public int storeValue(Value value) throws IOException {
		...
		byte[] valueData = value2data(this, value, true);
...
//		this.nativeSailStore.rocksDB.put(columnFamilyHandle,  );
	...
	private static byte[] value2data(ValueStore valueStore, Value value, boolean create) throws IOException {
		if (value instanceof IRI) {
			return valueStore.uri2data((IRI) value, create);
		} else if (value instanceof BNode) {
			return valueStore.bnode2data((BNode) value, create);
		} else if (value instanceof Literal) {
			return valueStore.literal2data((Literal) value, create);
		} else {
